...
class TestMetricsIntegration(TestBase):
    ...
    def test_prefetch_time_recorded(self):
        ...
        self.assertGreater(len(prefetch.data.data_points), 0)

    def test_prefetched_tasks_gauge_returns_to_zero(self):
        """After task completes, prefetched gauge should be back to zero."""
        self._run_task()
        metrics = self.get_sorted_metrics(SCOPE)
        prefetched = _find_metric(metrics, "flower.worker.prefetched.tasks")
        self.assertIsNotNone(prefetched)
        # After a complete task lifecycle the gauge should be 0
        self.assertEqual(prefetched.data.data_points[0].value, 0)
...
class TestTaskReceivedMetrics(TestBase):
    ...
    def test_task_received_records_event_and_prefetch(self):
        """Firing task_received should increment events_total and worker_prefetched_tasks."""
    ...
    def test_task_received_stores_received_time(self):
        """task_received should populate task_id_to_received_time."""
        request = _make_request(task_id="rcv-2")
        self.instrumentor._trace_task_received(request=request, sender=None)

        self.assertIn("rcv-2", self.instrumentor.task_id_to_received_time)
...
class TestTaskRevokedMetrics(TestBase):
    ...
    def test_task_revoked_decrements_prefetch(self):
        """Revoking a previously-received task should decrement prefetched counter."""
        request = _make_request(task_id="rev-2", hostname="celery@w1")
        sender = type("Sender", (), {"hostname": "celery@w1"})()

        # Receive first (increments prefetched)
        self.instrumentor._trace_task_received(request=request, sender=sender)
        # Then revoke (decrements prefetched)
        self.instrumentor._trace_task_revoked(request=request, sender=sender)
...
class TestPrefetchTimeGuards(TestBase):
    """Tests for _record_prefetch_time early-return paths."""
    ...
    def test_record_prefetch_time_none_task_name_is_noop(self):
        """None task_name should skip recording prefetch time."""
        self.instrumentor._record_prefetch_time("t1", None, "celery@w")
        metrics = self.get_sorted_metrics(SCOPE)
        prefetch = _find_metric(metrics, "flower.task.prefetch.time.seconds")
        self.assertTrue(
    ...
    def test_record_prefetch_time_none_worker_is_noop(self):
        """None worker should skip recording prefetch time."""
        self.instrumentor._record_prefetch_time("t1", "my.task", None)
        metrics = self.get_sorted_metrics(SCOPE)
        prefetch = _find_metric(metrics, "flower.task.prefetch.time.seconds")
        self.assertTrue(
            prefetch is None or len(prefetch.data.data_points) == 0
    ...
    def test_record_prefetch_time_no_received_time_is_noop(self):
        """No entry in task_id_to_received_time means nothing to record."""
    ...
    def test_record_prefetch_time_records_gauge(self):
        ...
        prefetch = _find_metric(metrics, "flower.task.prefetch.time.seconds")
        self.assertIsNotNone(prefetch)
        self.assertEqual(len(prefetch.data.data_points), 1)
...
class TestMemoryLeakPrevention(TestBase):
    ...
    def test_revoke_cleans_up_received_time(self):
        """After revoke, task_id_to_received_time should not retain the task_id."""
